...
exports.getSessionResults = function(req, res) {
    entities.session.findOne({ _id: req.params.sessionId }, function(err, session) {
        if (!err) {
            return res.json(logic.convertToResults(session));
        } else {
            return res.send(err);
        }
    });
...
exports.getAssessmentResults = function(req, res) {
    ...
    entities.session.findOne({ _id: req.params.sessionId, userId: req.user.email }, function(err, session) {
        if (!err) {
            var entity  = null;
            for (var i = 0; i < session.assessments.length; i++){
                if (session.assessments[i]._id == req.params.assessmentId){
                    entity =  session.assessments[i].results;
                    break;
                }
            }
...
exports.startAssessment = function(req, res) {
    var deferred = q.defer();
    if (req.user.type == 0){
        return res.send(401);
    }
    entities.session.findOne({_id: req.params.sessionId }, function(err, session){
        if (err) { return res.send(err); }
        for (var i = 0; i < session.assessments.length; i++){
            if (session.assessments[i]._id == req.params.assessmentId && session.assessments[i].start){
                session.assessments[i].start = moment.utc();
                session.markModified('assessments');
                break;
            }
        }
        session.save(function(err) {
            if (!err) {
                deferred.resolve("assessment", req.params.assessmentId);
                return res.send(204);
            } else {
                deferred.reject(err);
                return res.send(err);
...
exports.postSessionResult = function(req, res) {
    var deferred = q.defer();
    console.log(req.body);
    console.log(req.user);
    entities.session.update({
        _id: req.params.id,
        ...
        if (!err) {
            deferred.resolve("sessionResult");
            return res.send(200);
        } else {
            deferred.reject(err);
            return res.send(err);
...
exports.postAssessmentToSession = function(req, res) {
    var deferred = q.defer();
    if (req.user.type == 0){
        return res.send(401);
    }
...
exports.postAssessmentResult = function(req, res) {
    var deferred = q.defer();
    entities.session.findOne({ _id: req.params.sessionId }, function(err, session){
        var assessment;
        for (var i = 0; i < session.assessments.length; i++){
            if (session.assessments[i]._id == req.params.assessmentId && session.assessments[i].start){
                assessment = session.assessments[i];
                session.markModified('assessments');
                break;
            }
        }
        assessment.results.push(req.body);
        session.save(function(err){
            if (!err) {
...
exports.endAssessment = function(req, res) {
    var deferred = q.defer();
    if (req.user.type == 0 ){
        return res.send(401);
    }
    entities.session.findOne({_id: req.params.sessionId}, function(err, session){
        ...
        for (var i = 0; i < session.assessments.length; i++){
            if (session.assessments[i]._id == req.params.assessmentId && session.assessments[i].start){
                session.assessments[i].end = moment.utc();
                session.markModified('assessments');
            }
        }
        session.save(function(err) {
            if (!err) {
                deferred.resolve("assessment", session._id);
                return res.send(204);
            } else {
                deferred.reject(err);
                return res.send(err);
...
exports.deleteAssessmentFromSession = function(req, res) {
    var deferred = q.defer();
    if (req.user.type == 0){
        return res.send(401);
    }
    entities.session.findOne({_id: req.params.sessionId, userId: req.user.email}, function(err, session){
        if (!err && session) {
            ...
            for (var i = 0; i < session.assessments.length; i++){
                if (session.assessments[i]._id == req.params.assessmentId){
                    // remove it from the array.
                    session.assessments.splice(i, 1);
                }
            }
            // save the doc
            session.save(function(err) {
